  ...
  return (
    <div style={{ position: 'fixed',
      ...
      <div style={{ background: 'white',
        ...
        <div style={{ background: 'linear-gradient(90deg,#1A3A5C,#2A5278)',
                      padding: '16px 20px',
          ...
          <span style={{ 
                      ...
                      >{user ? 'Editar Usuario' : 'Nuevo Usuario'}</span>
        ...
        <div style={{ padding: 20, 
          ...
          <div>
            <label style={{ fontSize: 10, 
                      ...
                      color: '#6A9BB5', 
                      display: 'block', 
                      marginBottom: 6 }}>ROL</label>
            <div style={{ display: 'flex', gap: 8 }}>
              {['administrador', 'cajero'].map(r => (
                <button key={r} onClick={() => set('rol', r)} 
                      style={{ padding: '6px 20px',
                      borderRadius: 7, border: `1.5px solid ${form.rol === r ? '#1A3A5C' : '#E2F0F4'}`,
                      background: form.rol === r ? '#1A3A5C' : 'white',
                      color: form.rol === r ? 'white' : '#6A9BB5',
          ...
          <div style={{ display: 'flex', 
                      alignItems: 'center',
                      gap: 10, textTransform: 'capitalize'}}>
            <label style={{ fontSize: 10, 
                      fontWeight: 600,
                      color: '#6A9BB5' }}>ESTADO</label>
            <button onClick={() => set('activo', !form.activo)} 
                      style={{ padding: '5px 12px',
                      borderRadius: 7, 
                      border: `1.5px solid ${form.activo ? '#3DBD8A' : '#E05C6A'}`, 
      ...
      <div style={{ display: 'flex', 
        ...
        <div>
          <div style={{ fontSize: 24, 
                    fontWeight: 700, 
                    color: darkMode ? '#ffffff' : '#1A3A5C' }}>Usuarios</div>
          <div style={{ fontSize: 14, 
                    color: darkMode ? '#a0aec0' : '#6A9BB5' }}>Gestión de accesos al sistema</div>
        </div>
        <button onClick={() => 
          { setEditUser(null); setModal(true) }} 
        ...
        {usuarios.map(u => {
          ...
          return (
            <div key={u.id_usuario} style={{ 
                    ...
                    border: '1.5px solid #E2F0F4', 
                    padding: 16, 
                    boxShadow: '0 2px 8px rgba(11,31,75,.04)' }}>
              <div style={{ display: 'flex', 
                ...
                <div style={{ display: 'flex', 
                  ...
                  <div style={{  
                    ...
                    height: 42, 
                    borderRadius: 12, 
                    background: 'linear-gradient(135deg,#5BBFCC,#3A6E9E)', 
                    display: 'flex', 
                    alignItems: 'center', 
                    justifyContent: 'center', 
                    fontSize: 16, 
                  ...
                  <div>
                    <div style={{ 
                      fontSize: 18, 
                      fontWeight: 700, color: '#1A3A5C' }}>{u.nombre_completo}</div>
                    <div style={{ 
                      fontSize: 14,
                      color: '#6A9BB5' }}>@{u.nombre_usuario}</div>
                  </div>
                </div>
                <span style={{ fontSize: 14, 
                      ...
                      color: u.activo ? '#166534' : '#991B1B' }}>{u.activo ? 'Activo' : 'Inactivo'}</span>
              </div>
              <div style={{ display: 'flex', 
                      justifyContent: 'space-between', 
                      alignItems: 'center' }}>
                <span style={{ fontSize: 12, 
                      ...
                      padding: '3px 9px', 
                      borderRadius: 6,
                      background: u.rol === 'administrador' ? '#1A3A5C' : '#DFF4F7',
                      color: u.rol === 'administrador' ? 'white' : '#3A6E9E',
                      textTransform: 'capitalize' }}>{u.rol}</span>
                <button onClick={() => { setEditUser(u); setModal(true) }} 
                    style={{ 
                      ...
                      fontWeight: 600, 
                      cursor: 'pointer', 
                      fontFamily: 'inherit' }}> Editar</button>
              </div>
            </div>
          )
        })}
